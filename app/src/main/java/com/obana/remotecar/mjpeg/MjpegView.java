package com.obana.remotecar.mjpeg;
...
public class MjpegView extends SurfaceView implements SurfaceHolder.Callback, MjpegInputStream.OnStreamFinishedListener {
    ...
    public void startPlayback(TcpSocket tcpSocket) {
        String host = tcpSocket.getJpegMediaHost();
        int port = tcpSocket.getmJpegPort();
        String streamUrl =  String.format("http://%s:%d/?action=stream", host, port);

        new URLAsyncTask(this).execute(streamUrl);
    }
    ...
    private static class URLAsyncTask extends AsyncTask<String, Object, InputStream> {
        ...
        protected InputStream doInBackground(String... urls) {
            AppLog.i(TAG,"URLAsyncTask start...." );
            try {
                ...
                if (url.getProtocol().toUpperCase().equals("HTTPS")) {
                    ...
                    if (connection.getResponseCode() == HttpsURLConnection.HTTP_OK) {
                        AppLog.i(TAG,"mjpeg stream success:" + urls[0]);
                        return connection.getInputStream();
                    } else {
                        AppLog.e(TAG,"mjpeg stream failed:" + urls[0]);
                    }
                } else {
